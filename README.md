# Unitree_VLA_stack
This Parent Repo brings together the unitree vla and Simulation stack with all necessary submodel for a quick VLA test environment
...
unitree_sim_isaaclab	Isaac Lab Simulation, Image Server, DDS Bridge	Fichtl00/unitree_sim_isaaclab
unifolm-vla-inference-dds-shm-adapter	VLA Inference Code, DDS Client	Fichtl00/unifolm-vla-inference-dds-shm-adapter
xr_teleoperate/teleop/teleimager	Image Server für Kamera-Frames → SHM	unitreerobotics/xr_teleoperate
unitree_sdk2_python	DDS/SDK Utilities (unitree_sdk2py)	unitreerobotics/unitree_sdk2
...
Datenfluss
Simulation erstellt SHMs (isaac_head_image_shm, isaac_left_image_shm, isaac_right_image_shm, isaac_robot_state)

VLA Client hängt sich an SHMs an, liest Bilder + State

VLA Inferenz berechnet Action

Action wird in dds_robot_cmd SHM geschrieben

Simulation liest Action aus und wendet sie an

🚀 Quickstart – Isaac Lab Bridge (Dex1 / Stack Cube)
Start-Reihenfolge (wichtig!)
Simulation zuerst starten → erstellt SHMs

VLA Client zweitens starten → hängt sich an SHMs an
